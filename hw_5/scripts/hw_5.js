...
const area_cylinder = (h, r) => (2 * pi * (r * r)) + (2 * pi * r * h);
...
//
// // Не зовсім зрозуміла завдання, як саме повинен бути введений текст, зробила
// // двома варіантами
//
const createParagraph1 = (text) => {
    text = prompt('Введіть текст :', 'Hello');
    return '<p>' + text + '</p>'
}
document.write(createParagraph1())

const createParagraph2 = (text) => {
    return '<p>' + text + '</p>';
}
...
const createThreeUlWithNumber = (text, number) => {
...
const createThreeUlWithNumber_2 = (text, number) => {
    let ul = '<ul>';
    for (let i = 0; i < number; i++) {
        ul += '<li>' + text + '</li>';
    }
...
const numbers = [10, -5, 3.14, -7, 25, -12, 8.5, -4.7, 100,
    -50, 0.75, -2.3, 45, -30, 6.8, -9, 18, -15, 2.25, -3.8];
...
const swapFn = (arr, index1, index2) => {
...
document.write('<hr>')
// // - Написати функцію обміну валюти exchange(sumUAH,currencyValues,
// // exchangeCurrency)
...
const result = exchange(10000, [{ currency: 'USD', value: 40 },
